...
function doBeautify() {
    ...
    var value = elem.value,
        ...
        js = value.substring( start, end ),
        before = value.substring( 0, start ),
        after = value.substring( end, value.length );
        
    js = js.replace(rremoveindent, "");
    js = js_beautify(js);
    js = js.replace(rrestoreindent, "    ");
    elem.value = before + js + after;
    
    //Trigger the preview update
    var event = document.createEvent("KeyboardEvent");
    var method = event.initKeyEvent ? "initKeyEvent" : "initKeyboardEvent";
    event[method](                                                                                      
        "keypress",
...
chrome.extension.onRequest.addListener( function( request, sender, sendResponse ) {
    ...
    if( request && request.action === "beautify" ) {
        doBeautify();
    }
});